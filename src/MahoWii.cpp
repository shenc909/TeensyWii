...
// **********************
  ...
  //uint16_t GPS_HDOP;						           	// GPS HDOP      	 - Horizontal dilution of position
...
void annexCode() { // this code is executed at each loop and won't interfere with control loop if it lasts less than 650 microseconds
  ...
  if ( (calibratingA>0 && ACC ) || (calibratingG>0) ) { // Calibration
    LEDPIN_TOGGLE;
  } else {
...
void go_arm() {
  ...
  #endif
  #if GPS
    #if defined(ONLY_ALLOW_ARM_WITH_GPS_3DFIX)
    && (f.GPS_FIX && GPS_numSat >= 6)
    #endif
    && !rcOptions[BOXGPSHOME] && !rcOptions[BOXGPSNAV] // for safety RTH and NAV modes should be switched off
	&& !rcOptions[BOXGPSHOLD] && !rcOptions[BOXLAND] // for safety PH and LAND modes should be switched off
  #endif
    ) {
...
void loop () {
  ...
  static int16_t delta1[2], delta2[2];
	#ifdef KILL_SMALL_OSCILLATIONS
		static int16_t delta3[2];
	#endif
  ...
  if (updateTimer(&rcTimer, HZ2US(50))) {		// 50hz rc loop
    ...
    if(rcDelayCommand == 20) {
      if(f.ARMED) {                   // actions during armed
        ...
        if (rcSticks == THR_LO + YAW_LO + PIT_LO + ROL_CE) {    // GYRO calibration
          calibratingG=512;
          #if GPS && defined(RESET_HOME_AT_GYRO_CALIBRATION)
        ...
        if (rcSticks == THR_LO + YAW_HI + PIT_HI + ROL_CE) {            // Enter LCD config
          ...
          previousTime = micros();
                    ...
                    #ifndef DISABLE_YAW_ON_MIN_THROTTLE_DURING_AH
                      && !f.BARO_MODE
                    #endif
          ) go_arm();      // Arm via YAW
        #endif
    ...
    if (rcOptions[BOXARM] == 0) f.OK_TO_ARM = 1;
      ...
      if (f.ANGLE_MODE || f.HORIZON_MODE) {STABLEPIN_ON;} else {STABLEPIN_OFF;}
        ...
        #if GPS
        	//if (GPS_conf.takeover_baro) {
        		rcOptions[BOXBARO] = (rcOptions[BOXBARO] || f.GPS_BARO_MODE);
        	//}
        #endif
    ...
    if (f.ARMED) {                       //Check GPS status and armed
		...
		if (f.GPS_FIX) {
			...
			if (GPS_numSat >= 5) {
				...
				if(rcOptions[BOXGPSHOME]) { // if RTH activated reset other GPS modes to avoid restart RTH on switch of other modes

					mission_step.parameter1 = rcOptions[BOXLAND]; 		// IF BOXLAND activated then make auto landing after RTH
			...
			#ifdef AP_MODE
					bool isRollPitchInCenter = (abs(rcCommand[ROLL]) < AP_MODE) && (abs(rcCommand[PITCH]) < AP_MODE);
					if (rcOptions[BOXGPSHOLD]) { // if position hold activated reset hold position on ROLL/PITCH sticks movements
						rcOptions[BOXGPSHOLD] = isRollPitchInCenter;
					}
					//if (rcOptions[BOXLAND]) { // if land activated reset land and position on ROLL/PITCH sticks movements
					//	rcOptions[BOXLAND] = isRollPitchInCenter;
					//}
			#endif
				}
				...
				if (prv_gps_modes != gps_modes_check) { //Check for change since last loop
					...
					if (rcOptions[BOXGPSHOME]) { // RTH has the priority over everything else
						init_RTH();
				...
				//numSat dropped below 4, i.e. keep navigation if num sat =4 and reset if less
				if (GPS_numSat < 4) {
					if (f.GPS_mode != GPS_MODE_NONE) {
						if (f.GPS_BARO_MODE) {
							setAltToHold(alt.estAlt); // set current altitude to hold
						}
			...
			// GPS Fix disappeared, very unlikely that we will be able to regain it, abort mission
			if (f.GPS_mode != GPS_MODE_NONE) {
				if (f.GPS_BARO_MODE) {
					setAltToHold(alt.estAlt);    // set current altitude to hold
				}
				...
				f.GPS_mode = GPS_MODE_NONE;
				// NAV_state = NAV_STATE_NONE; // where GPS_reset_nav() has this
  ...
  static uint8_t taskOrder = 0; // never call all functions in the same loop, to avoid high delay spikes
	switch (taskOrder) {
		...
		case 3:
			taskOrder++;
			#if GPS
			  if (GPS_Compute()) break;  // performs computation on new frame only if present
			  #if defined(I2C_GPS)
...
#if BARO
  // apply altitude hold control
  bool isAltHoldControlApplied = applyAltHoldControl();
#endif

  // Gain throttle in case of inclination (only in angle and horizon mode)
#if defined(THROTTLE_ANGLE_CORRECTION)
  if(f.ANGLE_MODE || f.HORIZON_MODE) {
    ...
    if (!isAltHoldControlApplied && updateTimer(&gpsOutputTimer, HZ2US(GPS_OUTPUT_UPDATE_RATE))) {
    	...
    	//debug[3] = gpsOutputTimer.dTime/10;

		if ((f.GPS_mode != GPS_MODE_NONE) && f.GPS_FIX_HOME && (NAV_state != NAV_STATE_NONE)) {

		#ifdef INS_PH_NAV_ON

			if (isNavStateForPosHold()) {
				float dt = US2S(gpsOutputTimer.dTime); // dt in sec
				applyPosHoldPIDControl(&dt);
			}

			GPS_angle[ROLL] = (nav[LON] * cy - nav[LAT] * sy) / 10;
			GPS_angle[PITCH] = (nav[LON] * sy + nav[LAT] * cy) / 10;

		#else
			// Adds a rate control to nav output, will smoothen out nav angle spikes
			#define NAV_SLEW_RATE              60   // depends on GPS_OUTPUT_UPDATE_RATE

			nav_rated[LON] += constrain(wrap_18000(nav[LON] - nav_rated[LON]), -NAV_SLEW_RATE, NAV_SLEW_RATE);
			nav_rated[LAT] += constrain(wrap_18000(nav[LAT] - nav_rated[LAT]), -NAV_SLEW_RATE, NAV_SLEW_RATE);

			GPS_angle[ROLL] = (nav_rated[LON] * cy - nav_rated[LAT] * sy) / 10;
			GPS_angle[PITCH] = (nav_rated[LON] * sy + nav_rated[LAT] * cy) / 10;
  ...
  //Used to communicate back nav angles to the GPS simulator (for HIL testing)
    ...
    SerialWrite(GPS_SERIAL, 0xa5);
    SerialWrite16(GPS_SERIAL, nav[LAT]+rcCommand[PITCH]);
    SerialWrite16(GPS_SERIAL, nav[LON]+rcCommand[ROLL]);
    SerialWrite16(GPS_SERIAL, (nav[LAT]+rcCommand[PITCH])-(nav[LON]+rcCommand[ROLL])); //check
  #endif
  ...
  // PITCH & ROLL
    ...
    error = (int16_t)(((int32_t)(rc - imu.gyroData[axis]) * cycleTime)>>12); // normalized to cycle time = 4096
	...
	// delta = ((int32_t)delta * ((uint16_t) 0xFFFF / (cycleTime >> 5))) >> 9; // normalized to cycle time = 4096
  ...
  errorGyroI_YAW  += mul((int16_t)(((int32_t)error * cycleTime)>>12), conf.pid[YAW].I8); // normalized to cycle time = 4096
